...
if (!K5 || !K6 || !TYPES) {
  process.stderr.write("Usage: node probe.mjs <KIRBY_K5_ROOT> <KIRBY_K6_ROOT> <KIRBY_TYPES_ROOT>\n");
  process.exit(1);
}
...
function historyReach(root) {
  ...
  try {
    const shallow = run(`git -C "${root}" rev-parse --is-shallow-repository`).trim() === "true";
...
for (const [label, root, h, ver] of [["K5", K5, reach.k5, k5v], ["K6", K6, reach.k6, k6v]]) {
  if (ver === "unknown") continue;
  if (h.shallow || h.minorTagLines < 3) {
    postureFlags.push(`SHALLOW-HISTORY: ${label} root ${root} (shallow=${h.shallow}, ${h.minorTagLines} minor-tag line(s)) cannot date @since reliably -> run @since archaeology against a full-history checkout (git tag --contains against the deepest available root, or gh upstream). Never date a pre-floor member here.`);
  }
}
